...
**當前主要狀態**：
- **2026-09-24（#469965）**：`drop_eval_datasets_agent_mode.sql`（DROP COLUMN eval_datasets.agent_mode；ORM 早已移除、prompt_optimizer raw SQL 同分支移除）依 Larry「兩個都授權」套 `local-docker`（紀錄 29 筆）與 `company-poc-vm`（紀錄 74 筆），各驗證 information_schema 0 列 + `_applied_migrations`（applied_by=claude-dev）。
- **2026-09-23（#107）**：`alter_bots_line_credentials_text.sql`（bots.line_channel_secret / line_channel_access_token VARCHAR(255) → TEXT）依 Larry「授權」套 `local-docker`（紀錄 28 筆）與 `company-poc-vm`（紀錄 73 筆），各驗證 information_schema data_type=text + `_applied_migrations`（applied_by=claude-dev）。
