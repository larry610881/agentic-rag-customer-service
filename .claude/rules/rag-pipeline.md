...
paths:
  ...
  - "apps/backend/src/infrastructure/milvus/**"
...
graph TD
    ...
    P5 --> P6["Milvus upsert<br/>collection=kb_{kb_id}<br/>tenant_id field (CRITICAL)"]
...
graph LR
    ...
    E --> S["Milvus Search<br/>(tenant_id filter CRITICAL)"]
    S --> A["Augment Prompt"]
    A --> L["LLM Generate"]
```
...
- **所有向量搜尋必須包含 `tenant_id` 過濾條件**（Milvus filter expression `tenant_id == "..."`）
- Milvus collection 命名 `kb_{kb_id}` (per-KB，**非** per-tenant)，filter expression **必須**帶 `tenant_id`
- `tenant_id` 是 first-class field 且建 INVERTED index（避免 full scan）
...
- 維度變更時 Milvus 會自建新 collection（舊的不動）— `_build_schema(vector_size)` 看 dim 決定
- 向量文本組裝：若有 `context_text` 則 `"{context_text}\n\n{content}"`，否則純 `content`

### Milvus Schema 設計（CRITICAL — strong-typed，不是 dynamic）

`infrastructure/milvus/milvus_vector_store.py::_build_schema()` 定義：

```python
fields = [
    FieldSchema(name="id", dtype=VARCHAR, max_length=128, is_primary=True),
    FieldSchema(name="vector", dtype=FLOAT_VECTOR, dim=vector_size),
    FieldSchema(name="tenant_id", dtype=VARCHAR, max_length=64),    # INVERTED
    FieldSchema(name="document_id", dtype=VARCHAR, max_length=64),  # INVERTED
    FieldSchema(name="content", dtype=VARCHAR, max_length=65535),
    FieldSchema(name="chunk_index", dtype=INT64),
    FieldSchema(name="content_type", dtype=VARCHAR, max_length=64),
    FieldSchema(name="language", dtype=VARCHAR, max_length=16),
    FieldSchema(name="extra", dtype=JSON),  # 任意 metadata 進這
]
enable_dynamic_field=False  # CRITICAL — 不能臨時加欄位
```

**設計紅線**：
- `enable_dynamic_field=False` → 加新 first-class field 必須走 schema migration（`add_field()`）+ 對既有 collection 重建索引
- 新 metadata 預設塞 `extra` JSON 欄位（在 `_KNOWN_FIELDS` 之外的 payload key 自動進 `extra`）
- 高頻 filter 欄位才升 first-class（INVERTED index），低頻可留 `extra` JSON
- Filter expression 走字串 DSL（`tenant_id == "T001" and document_id == "..."`），透過 `_build_filter_expr()` 組裝
- **Filter value 必須過 `_sanitize_filter_value()`**（防 expression injection）— 新增 filter 入口必查
...
### 6. Milvus Upsert
- Collection: `kb_{kb_id}`（hyphen `-` 自動轉 `_`，見 `_safe_collection_name`）
- First-class fields: `tenant_id`, `document_id`, `content`, `chunk_index`, `content_type`, `language`
- 其他 payload key 自動進 `extra` JSON 欄位（`_KNOWN_FIELDS` 以外的）
- 冪等：相同 `id` (chunk_id) 自動覆蓋（`MilvusClient.upsert`）
...
流程：
1. 從 Milvus 讀 KB 所有 chunk 向量（`fetch_vectors` 或 `query`）
2. 聚類（K-means 或相似度）
3. LLM 為每群生成中文分類名
...
| Chunking 完 | — | — | 75% |
| Contextual 完 | — | — | 80% |
| Embedding + Milvus 完 | **`processed`** | **`completed`** | **100%** |
...
| MilvusVectorStore | Infrastructure | `infrastructure/milvus/milvus_vector_store.py` |
...
mock_milvus.search = AsyncMock(return_value=[
    SearchResult(id="1", score=0.95, payload={"content": "退貨政策", "tenant_id": "T001"}),
])
```

### Integration Test：Docker Milvus
- 使用 docker-compose.yml 中的 Milvus 實例（standalone or with etcd/minio）
- 每個測試前 drop_collection 重建（schema 是 strong-typed 不能 patch 既有 collection）
- 確認 INVERTED index 對 `tenant_id` / `document_id` 已建（避免 full scan 跑超慢）

### 加 first-class field 的 schema migration
- `_build_schema()` 改 + 改 `_KNOWN_FIELDS`
- 對既有 collection 走 `MilvusClient.add_field()` (Milvus 2.4+) + `create_index` 加 INVERTED
- backfill 既有 row（需要的話用 `query` 拿 ID 後 batch upsert 補欄位）
- 寫專屬 migration script 在 `scripts/` 下
...
| # | 項目 | 風險 | 建議 |
|---|------|------|------|
| 1 | Milvus collection 非 per-tenant（per-KB）| 搜尋漏 `tenant_id` filter 會跨租戶洩漏 | 所有 search 入口必須 test tenant isolation；filter expression 必走 `_sanitize_filter_value` |
