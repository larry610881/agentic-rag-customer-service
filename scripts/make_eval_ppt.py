...
def slide_verdict(prs):
    ...
    rows = [
        ...
        ["回應速度（測試機）", "不明顯劣於雲端", "2.20 秒　通過", "5.50 秒　未通過", "1.77–1.80 秒"],
    ...
    text(s, 1.15, 5.90, 11.0, 0.55,
         "品質、工具、結構化輸出三項與硬體無關，結論可直接沿用；回應速度量自租用的測試機，"
         "換到公司自有設備需重測（下一步第一項）。模型選擇的結論不受影響：MoE 架構在記憶體頻寬"
         "越受限的機器上，領先 dense 越多。",
         size=10.5, color=INK)
    return s
...
def slide_latency(prs):
    ...
    bullets(s, 0.95, 4.05, 11.4, 1.4, [
        ...
        ("這個數字綁在測試用的 GPU 上　",
         "RTX PRO 6000（96GB、GDDR7）。產字速度由記憶體頻寬決定，換設備必須重測，已列為下一步第一項。"),
    ], size=11, gap=0.46)
...
def slide_gx10(prs):
    ...
    card(s, 6.80, 4.32, 5.60, 1.15, "兩個不受推估誤差影響的結論", [
        "第一，dense 27B 徹底出局，差距太大，誤差吃不掉。",
        "第二，4-bit 量化是必要條件而非選項，7.4 秒到 4.3 秒全靠它。",
    ...
    text(s, 1.15, 5.80, 11.0, 0.55,
         [("要把推估變成實測，只需要兩件事：", {"bold": True, "color": INK}),
          ("一是自有設備的實際記憶體頻寬與是否支援 FP4 原生運算，"
           "二是在該設備起一個推論服務並開一個埠。量測工具已完備，半天可得三方對照。", {})],
         size=10.5, color=BODY)
    return s
...
def slide_cost(prs):
    s = content_slide(prs, "成本：設備已在手上，算法要換")
    text(s, 0.92, 1.48, 11.48, 0.30,
         "本次評測用的是按小時租用的 GPU，那是驗證期的計價方式。公司已有 GX10，"
         "長期成本應以「折舊＋電費＋維運」計算，以下重新試算。",
         size=11, color=BODY)

    rows = [
        ["供應方式", "月固定成本", "每輪成本（以月 5 萬輪計）", "成本組成"],
        ["雲端 API（gpt-5.6-luna）", "無", "US$0.0017", "用多少付多少"],
        ["雲端 API（gpt-5.6-terra）", "無", "US$0.0170", "用多少付多少"],
        ["租用 GPU（本次評測用）", "約 US$1,505", "約 US$0.0301", "僅供驗證，非長期方案"],
        ["新購設備（3 年攤提）", "約 US$123", "約 US$0.0025", "折舊 US$111 ＋ 電費 US$12"],
        ["公司現有 GX10", "約 US$12", "約 US$0.0002", "只有電費，設備已購置"],
    ]
    table(s, rows, 0.92, 1.88, 11.48, 1.98, [3.20, 2.10, 3.05, 3.13],
          head_size=10.5, body_size=10.5, row_h=0.32, emphasis={5: GREEN})

    text(s, 0.92, 4.02, 11.48, 0.30, "但維運人力才是真正的變數（上表未含）",
         size=13, color=ORANGE, bold=True)
    rows2 = [
        ["每月維運投入", "月固定合計", "每輪成本", "與雲端 luna 相比", "損益平衡輪數"],
        ["沿用現有人力（不另計）", "US$12", "US$0.0002", "便宜 8 倍", "0.7 萬輪／月"],
        ["約 US$100／月", "US$112", "US$0.0022", "略貴", "6.6 萬輪／月"],
        ["約 US$300／月", "US$312", "US$0.0062", "貴 3.6 倍", "18.4 萬輪／月"],
    ]
    table(s, rows2, 0.92, 4.42, 7.30, 1.26, [2.35, 1.25, 1.25, 1.30, 1.15],
          head_size=9.5, body_size=10, row_h=0.30)

    card(s, 8.55, 4.42, 3.85, 1.26, "兩個數字要補", [
        "GX10 的實際採購價（上表以 US$4,000、3 年攤提試算）",
        "每月維運投入的人力估計",
    ], heading_color=ORANGE, heading_size=11.5, body_size=9.5)

    box(s, 0.92, 5.85, 11.48, 0.82, shape=MSO_SHAPE.ROUNDED_RECTANGLE,
        fill=CARD, radius=0.08)
    text(s, 1.15, 6.00, 11.0, 0.55,
         [("結論：設備成本不是問題，維運人力才是。", {"bold": True, "color": INK}),
          ("GX10 已購置，用它的邊際成本只有電費；是否划算完全取決於要投入多少維運人力，"
           "以及月流量能否達到損益平衡點。本次評測本身只花了 US$21.5。", {})],
         size=10.5, color=BODY)
    return s
...
def slide_credibility(prs):
    ...
    rows = [
        ...
        ["防護攔截時回純文字、破壞 JSON 契約",
         "結構化輸出五款分數完全相同，看起來像模型都一樣差；實情是攔截回合模型根本沒被呼叫"],
    ]
...
def slide_next(prs):
    ...
    bullets(s, 0.95, 3.58, 11.4, 1.9, [
        ("在公司自有設備上重測回應速度　",
         "量測工具已完備，只需在該設備起推論服務並開一個埠，半天可得三方對照數字，零額外成本。"),
        ("補足多工具串接樣本　",
