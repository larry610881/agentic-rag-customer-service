...
def slide_cost(prs):
    ...
    text(s, 0.92, 1.48, 11.48, 0.30,
         ...
         "長期成本應以「電費＋維運」計算，以下重新試算。",
         size=11, color=BODY)

    rows = [
        ["供應方式", "月固定成本", "每輪成本（以月 5 萬輪計）", "說明"],
        ["雲端 API（gpt-5.6-luna）", "無", "US$0.0017", "用多少付多少，雲端最省的一款"],
        ["雲端 API（gpt-5.6-terra）", "無", "US$0.0170", "用多少付多少"],
        ["租用 GPU（本次評測用）", "約 US$1,505", "約 US$0.0301", "僅供驗證，非長期方案"],
        ["新購設備（3 年攤提）", "約 US$123", "約 US$0.0025", "未來擴充時才需評估"],
        ["公司現有 GX10", "約 US$12（電費）", "約 US$0.0002", "設備已購置，只計邊際成本"],
    ]
    table(s, rows, 0.92, 1.88, 11.48, 1.86, [3.20, 2.20, 3.05, 3.03],
          head_size=10.5, body_size=10.5, row_h=0.30, emphasis={5: GREEN})

    text(s, 0.92, 3.90, 11.48, 0.30, "真正要決定的是：維運投入低於多少，地端才划算",
         size=13, color=ORANGE, bold=True)
    rows2 = [
        ["月流量", "每月維運預算上限", "約合人天", "超過上限的意思"],
        ["1 萬輪", "US$5", "0.03 人天", "純比成本，雲端必勝"],
        ["5 萬輪", "US$73", "0.5 人天", "半天人力以內才划算"],
        ["10 萬輪", "US$158", "1.0 人天", "撐得起 1 人天維運"],
        ["30 萬輪", "US$498", "3.2 人天", "地端成本優勢明顯"],
    ]
    table(s, rows2, 0.92, 4.28, 7.15, 1.56, [1.35, 2.05, 1.45, 2.30],
          head_size=9.5, body_size=10, row_h=0.30, emphasis={3: GREEN})

    card(s, 8.40, 4.28, 4.00, 1.56, "採購價為何不進決策", [
        "設備已購置，採購價屬沉沒成本，多用一次不會多付。",
        "決策相關的只有邊際成本：電費與維運。",
        "採購價只在評估「要不要加買」時才需要。",
    ], heading_size=11.5, body_size=9.5)

    box(s, 0.92, 5.98, 11.48, 0.70, shape=MSO_SHAPE.ROUNDED_RECTANGLE,
        fill=CARD, radius=0.08)
    text(s, 1.15, 6.10, 11.0, 0.48,
         [("上限公式：月流量 × US$0.0017（雲端最省單價）－ US$12（電費）。",
           {"bold": True, "color": INK}),
          ("全部由實測單價推導，唯一的假設是人天成本以 NT$5,000 計，"
           "改這個數字只影響最右欄換算，不影響結論。", {})],
         size=10.5, color=BODY)
    return s
