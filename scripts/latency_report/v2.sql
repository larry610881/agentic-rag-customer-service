SELECT to_char(t.created_at AT TIME ZONE 'Asia/Taipei', 'HH24:MI:SS') AS tm,
       max(n->'metadata'->>'message_preview') FILTER (WHERE n->>'node_type'='user_input') AS q,
       round(t.total_ms/100)/10.0 AS total_s,
       count(*) FILTER (WHERE n->>'node_type'='agent_llm') AS llm_calls,
       count(*) FILTER (WHERE n->>'node_type'='tool_call') AS tool_calls,
...
WHERE t.source='line' AND t.created_at > timestamp '2026-07-23 06:33:00+00'
