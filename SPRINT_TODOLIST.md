...
| [#68](https://github.com/larry610881/agentic-rag-customer-service/issues/68) | P7 異常使用者分級控管（Abuse Score + Tiered Response）：P7a–P7d 前後端全部完成 ✅ 2026-09-04、四支 migration 兩環境已套，待整包部署 | `enhancement` | 3d-711 session 計畫書，10 月紅隊複測 |
...
| [#74](https://github.com/larry610881/agentic-rag-customer-service/issues/74) | 雙軌計價：方案可選 token 制（預設）或點數制（月費 / 月點數 / 類別倍率 / 模型點數），token 仍為事實來源；依賴 #73 | `enhancement` | ✅ 2026-09-07 前後端完成（待 migration 授權 + 部署）：方案 billing_mode / 月點數 / 加購點數 / 類別倍率表 / 用盡策略（自動展延 或 用完即擋、租戶可否自改、展延月上限、寬限、被擋文案）、billing_settings 匯率、model_pricing 點數、usage.points + reasoning_tokens、QuotaPreflightService（Redis 30s、fail-open、三通路 + 文件 + 評估入口，402 / SSE quota_exhausted / LINE 文案）、租戶 billing-policy 端點（方案不允許 403）；前端方案頁計價與策略區、匯率卡、倍率表、定價點數、租戶配額頁點數儀表與策略切換、租戶覆寫、聊天被擋提示；points_billing 15 + quota_exhaustion_policy 25 scenarios，unit 1935、前端 466 綠、mypy 195；四支 migration 待授權；未做：全租戶配額列表點數欄、點數制 80/100% 提醒 |
| [#75](https://github.com/larry610881/agentic-rag-customer-service/issues/75) | 防護階段三層設定（系統底線 / 方案 / 租戶 bot 自選）+ 鎖定 + 全稽核；kb 模式攻擊判定開關 | `enhancement` | ✅ 2026-09-07 前後端完成（待 migration 授權 + 部署）：`domain/settings/layered` 通用三層（abuse 改用、行為不變）、`domain/security/guard_stages`（regex_input / classifier_attack / output_guard / abuse_scoring / local_classifier 預留；底線 ∪ 方案 ∪ 租戶加嚴 ∪ bot 超集；鎖定；DB 失效 fail-safe 全開）、`GuardPipeline` 三通路共用、kb 模式攻擊判定 = 不帶 worker 的 classify_sanitize、內建方案 standard / exhibition、全寫稽核且租戶變更紀錄顯示「平台」；前端 `/admin/guard-control` 四分頁、`/guard-status`、bot 表單防護階段區；guard_stages 23 scenarios（42 cases），unit 1977、前端 494 綠、mypy 195；migration `add_guard_settings.sql` 待授權 |
...
| [#77](https://github.com/larry610881/agentic-rag-customer-service/issues/77) | worker 稽核補 tenant / bot 連結進租戶變更紀錄 + 設定變更通知（管理員可設定） | `enhancement` | ✅ 2026-09-07 前後端完成（待 migration 授權 + 部署）：audit_logs parent_entity 連結（worker 稽核帶 tenant_id + parent=bot，租戶變更紀錄併 worker 列與 entity_name）、設定變更通知（欄位群組 model / prompt / knowledge / tools / guard，租戶偏好端點，渠道 notify_config_change，AuditRecorder on_recorded fail-open 直接派送，平台變更也通知）；前端 worker 欄位對照、變更紀錄 worker 徽章、渠道開關、`/tenant-settings` 設定變更通知卡；bot_audit_visibility +5、config_change_notification 17 scenarios（26 cases）+ #75 guard_settings_api 8 個整合測試；unit 2008、前端 510 綠、mypy 195；migration `add_audit_parent_entity_and_config_change_notify.sql` 待授權 |
