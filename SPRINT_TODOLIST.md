...
| [#97](https://github.com/larry610881/agentic-rag-customer-service/issues/97) | 契約改造第一批（1–6）：`ApiDateTime` 單一 profile（23 個回應欄位）、`ErrorResponse`/`ValidationErrorResponse` 進 OpenAPI（app 層 responses）、六支對外 router 65 處 `HTTPException` → `ApiError(code=…)`、9 個可空陣列的 null 語意寫進 schema、`scripts/export_openapi.py` + `docs/api/openapi.json` 快照測試 + `make openapi`、CORS expose 五個標頭 | `enhancement` | 09-16 程式完成於分支，待部署；`api_contract_batch1.feature` 7 scenarios；前端 513 綠 |
