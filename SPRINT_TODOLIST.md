...
| [#99](https://github.com/larry610881/agentic-rag-customer-service/issues/99) | 契約另案總收尾：串流冪等 + Last-Event-ID 重播、生成中斷線部分計費（estimated，**migration 待授權**）、usage 金額字串、全 router 143 處 ApiError、not_found 常數表、串流三段拆分、廠商交付包、VM 同步、channel-parity 二-2 trace 合一 / 二-3 攔截回應合一 / 二-6 LINE 接記憶 / 二-7 回放保真度標注 / 二-8 ADR-0003；GitHub 盤點關閉 10 個已完成 Issue | `enhancement` | 09-16 程式完成於分支；**待 Larry：migration 兩環境授權、測試對話 70 筆 DELETE 確認**，之後部署；CI 的 oasdiff 留給 CI/CD 階段 |
