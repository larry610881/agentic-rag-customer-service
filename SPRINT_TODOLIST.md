...
| **External Producer Integration — Source Tracking (Phase 1)** | **✅ 完成（待 dev-vm + local-docker Milvus migration）** | **100% 程式碼** | **Issue [#44](https://github.com/larry610881/agentic-rag-customer-service/issues/44) Phase 1 of 3: agentic-rag SaaS 對外整合基礎. Milvus schema 升級 — `_KNOWN_FIELDS` 加 source/source_id + `_build_schema()` 加兩個 first-class VARCHAR field（default ""）+ `ensure_collection` 對新 collection 加 INVERTED index. `_build_filter_expr` 從 `==` 擴充支援 `IN` operator（list value → `field in [...]`），每個 element 仍走 `_sanitize_filter_value` 防注入. 新 `DeleteDocumentsBySourceUseCase`（DDD application 層, ensure_kb_accessible 雙重 tenant 檢查）+ `DELETE /knowledge-bases/{kb_id}/documents/by-source` endpoint（Pydantic v2 validator: source 1-64 / source_ids 1-1000，路由順序在 `/{doc_id}` 之前）. Migration script `scripts/add_source_fields_to_milvus.py`（idempotent: describe → release → add_field with default_value="legacy"/"" → INVERTED index → load）+ `--dry-run` 預覽. 5 BDD integration scenarios（source_tracking.feature: 單 id / 多 id IN / 跨租戶 404 / KB 404 / 空 list 422）+ 10 unit tests（filter DSL + schema fields）+ 868 既有 unit tests 不退步. Architecture journal 已寫. ⚠️ TODO: local-docker + dev-vm Milvus migration script 套用 + Phase 2 (Bulk Ingest) + Phase 3 (Unified Search)** |
