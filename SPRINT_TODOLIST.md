...
| **康達盛通 POC 反饋 P0 修復（WP-A/H/B2 + B1/C SQL 備妥）** | **🔄 進行中** | **70%** | **Plan `.claude/plans/poc-feedback-fix-plan.md`。8 題根因全數線上 trace 實證（trace 30 筆 + Milvus 逐鍵比對）。WP-A 問題7：dm_image_query_tool context 改用去重前結果組裝（包大人同頁被 by_doc dedup 砍出 context）+ 2 regression + BDD feature。WP-H 問題2：flex_contact_builder 按鈕 URL 加 openExternalBrowser=1（Android LINE WebView 不彈授權致網路電話卡住）+ 5 tests。WP-B2 問題4：LINE 出口 strip_markdown_for_line 安全網 + 9 tests + prompt_defaults seed 加格式/連結規則。1049 unit tests pass。⏳ 待辦：migrations/poc_feedback_prompt_and_guard_fixes.sql 套 dev-vm（待 Larry preview 確認）、GitHub issues（public repo 待決）、P1 波（WP-D/E/F）、Android 實機驗證** |
