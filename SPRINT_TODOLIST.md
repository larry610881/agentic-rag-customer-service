...
| ~~[#66](https://github.com/larry610881/agentic-rag-customer-service/issues/66)~~ | ~~需求三收尾：worker direct_retrieval 進 API/UI + bot mode profile~~ | ~~`enhancement`~~ | ~~需求書 §三~~ ✅ 程式完成 2026-09-02，**migration `add_bot_mode.sql` 待三環境套用** |
...
| **需求三收尾：bot mode profile（Issue #66）** | **⚠️ 程式完成，migration 待套** | **90%** | **2026-09-02：`Bot.mode`（fast/deep，快照白名單 + 指紋 extra）；fast = 無 worker 也走快速道、rerank/rewrite/HyDE 關、升級 ReAct 工具上限 2；deep = worker 快速道依設定（修正 #61 全域關 rerank）；worker CRUD API + 後台開關；bot 表單 mode 選擇；`FAST_LANE_ALLOW_RERANK` 移除；需求書 §三/§四 依現況更新。`fast_lane_profile.feature` 11 scenarios，unit 1472 全綠。待：`add_bot_mode.sql` 三環境授權套用** |
