...
| [#98](https://github.com/larry610881/agentic-rag-customer-service/issues/98) | 契約改造第二批（7–11）：五支建立型端點 `Idempotency-Key`（含 multipart 上傳指紋，標頭進 OpenAPI）、`ApiMoney` 24 個金額欄位固定 6 位小數、無型別 object 標 `x-opaque` + 對話歷史 `structured_content` typed、`X-Client-Version` 中介層（`min_client_version`，低於門檻 426）、SSE `id:` 序號 + `docs/api/sse-contract.md` + widget 非串流 `POST /widget/{code}/chat` | `enhancement` | 09-16 程式完成於分支，待部署；`idempotent_creates.feature` 4 + `api_contract_batch2.feature` 8 scenarios；`Last-Event-ID` 重播與串流冪等同一觸發條件另案 |
