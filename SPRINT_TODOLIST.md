...
- ✅ **Phase C 閘門引擎**（開工前三路探索定案，計畫 `.claude/plans/prompt-gate-phase-c-plan.md`）
  - ✅ 三層開關：bots gate_* 六欄（8 落點鏈）+ tenants.prompt_gate_enabled + 前置條件（綁題集）
  - ✅ Verdict Engine（硬/軟/預算、by case_id 聚合）+ 26 斷言 hard/soft 映射（params 可覆寫）
  - ✅ config_override 影子執行 + test_mode 六面隔離 + history_override（= Playground 後端）
  - ✅ StartGateRun（五連前置檢查）→ 背景執行（Round 策略）→ 逐題報告（4KB 截斷 + trace nodes）
  - ✅ 狀態機接線：validating→pending_publish/draft、block/warn/force 發布分支、孤兒清理
...
| ~~[#57](https://github.com/larry610881/agentic-rag-customer-service/issues/57)~~ | ~~Trace 瀑布圖（時間軸視圖）：請求邊界根節點 + 檢索分段儀表 + 前端甘特列~~ | ~~`enhancement`~~ | ~~09-02 審計盤點~~ ✅ 2026-09-02 |
| [#63](https://github.com/larry610881/agentic-rag-customer-service/issues/63) | 延遲聚合 bot_daily_metrics + 診斷規則改接營運訊號 | `enhancement` | #59/#57 後續 |
| [#65](https://github.com/larry610881/agentic-rag-customer-service/issues/65) | integration 測試本機 41 失敗（環境/fixture） | `bug` | 09-02 依賴升級驗證發現 |
| [#68](https://github.com/larry610881/agentic-rag-customer-service/issues/68) | P7 異常使用者分級控管（Abuse Score + Tiered Response）：P7a–P7d 前後端全部完成 ✅ 2026-09-04，已上線 rev 00012-pnt | `enhancement` | 3d-711 session 計畫書，10 月紅隊複測 |
| [#70](https://github.com/larry610881/agentic-rag-customer-service/issues/70) | 知識庫問答模式（bot mode kb）+ 結構化輸出 output_format（text / plain_text / json + schema）+ 供應商 JSON 能力等級提醒；3D 虛擬展串接備案（Gemini 3.7 Flash 單模型）驗收案例 | `enhancement` | ✅ 2026-09-04 前後端完成（前端 819b491、後端本 commit）：kb_mode 13 + structured_output 37 scenarios，unit 1744 綠、前端 390 綠；migration 兩環境已套；待部署 POC 與 3D 展兩個 prompt 實測 |
| [#71](https://github.com/larry610881/agentic-rag-customer-service/issues/71) | bot 儲存前變更簡述確認 + 租戶端 bot 變更紀錄分頁（稽核已有但只有 system_admin 看得到） | `enhancement` | ✅ 2026-09-07 完成：`GET /bots/{id}/audit-logs`（租戶範圍、跨租戶 404、keyset 分頁、llm_params 攤平、長文字只報字數）、儲存前「確認變更」對話框、bot 頁「變更紀錄」區塊；bot_audit_visibility 10 scenarios，unit 1816、前端 418 綠；worker 稽核無 bot_id 連結故只列 bot 本體；變更通知未做（可選）；未部署 |
| [#72](https://github.com/larry610881/agentic-rag-customer-service/issues/72) | 推理強度可關閉（none）+ web/widget/API 漏傳修正 + 各供應商 thinking 對應 + 審計/指紋/reasoning token；weekly-report session 轉派 | `enhancement` | ✅ 2026-09-04 完成：推理強度加「關閉（none）」、三通路 llm_params 對齊、各供應商對應（gpt-5.x none 直傳 / Gemini 直傳 / Anthropic 4.6+ adaptive+effort、Opus 5 明確 disabled）、trace 記 requested/effective、reasoning_tokens 進 trace JSON；reasoning_effort_control 18 scenarios（43 cases），unit 1806 綠、前端 394 綠；未部署（Larry 自己 push/deploy）；gpt-5.1 預設 none → LINE 之前沒走 thinking |
| [#73](https://github.com/larry610881/agentic-rag-customer-service/issues/73) | 用量記帳缺口：embedding 從未入帳（Cached/Dynamic 包裝沒轉發 token）、reprocess 管線全未記、查詢 embedding 未記、bot_id 歸屬遺失、字元數當 token、死類別；點數制前置 | `bug` | ✅ 2026-09-07 完成：EmbeddingService 回傳用量（快取/動態層透傳）、process/reprocess 共用記帳 helper、query_embedding / dm_metadata 新類別、輔助 LLM 帶 bot_id、rag/guard 標 deprecated 拒新寫入、字串字面值改 enum + 靜態守門、RAGEvaluation 死碼拆除；usage_accounting_coverage 19 scenarios，unit 1880 綠、mypy 197；`docs/token-usage.md`；未部署 |
| [#74](https://github.com/larry610881/agentic-rag-customer-service/issues/74) | 雙軌計價：方案可選 token 制（預設）或點數制（月費 / 月點數 / 類別倍率 / 模型點數），token 仍為事實來源；依賴 #73 | `enhancement` | ✅ 2026-09-07 完成並上線 rev 00013-75s：方案 billing_mode / 月點數 / 加購點數 / 類別倍率表 / 用盡策略（自動展延 或 用完即擋、租戶可否自改、展延月上限、寬限、被擋文案）、billing_settings 匯率、model_pricing 點數、usage.points + reasoning_tokens、QuotaPreflightService（Redis 30s、fail-open、三通路 + 文件 + 評估入口，402 / SSE quota_exhausted / LINE 文案）、租戶 billing-policy 端點（方案不允許 403）；前端方案頁計價與策略區、匯率卡、倍率表、定價點數、租戶配額頁點數儀表與策略切換、租戶覆寫、聊天被擋提示；points_billing 15 + quota_exhaustion_policy 25 scenarios，unit 1935、前端 466 綠、mypy 195；四支 migration 兩環境已套；未做：全租戶配額列表點數欄、點數制 80/100% 提醒 |
| [#75](https://github.com/larry610881/agentic-rag-customer-service/issues/75) | 防護階段三層設定（系統底線 / 方案 / 租戶 bot 自選）+ 鎖定 + 全稽核；kb 模式攻擊判定開關 | `enhancement` | ✅ 2026-09-07 完成並上線 rev 00013-75s：`domain/settings/layered` 通用三層（abuse 改用、行為不變）、`domain/security/guard_stages`（regex_input / classifier_attack / output_guard / abuse_scoring / local_classifier 預留；底線 ∪ 方案 ∪ 租戶加嚴 ∪ bot 超集；鎖定；DB 失效 fail-safe 全開）、`GuardPipeline` 三通路共用、kb 模式攻擊判定 = 不帶 worker 的 classify_sanitize、內建方案 standard / exhibition、全寫稽核且租戶變更紀錄顯示「平台」；前端 `/admin/guard-control` 四分頁、`/guard-status`、bot 表單防護階段區；guard_stages 23 scenarios（42 cases），unit 1977、前端 494 綠、mypy 195；migration 已套兩環境 |
| [#76](https://github.com/larry610881/agentic-rag-customer-service/issues/76) | Anthropic 依模型決定是否送 temperature（Opus 4.7+ / Opus 5 / Fable 5 會 400；官方文件） | `bug` | ✅ 2026-09-07 完成 f35f80f：sampling_params_allowed 表、三處套用、41 測試；未部署 |
| [#77](https://github.com/larry610881/agentic-rag-customer-service/issues/77) | worker 稽核補 tenant / bot 連結進租戶變更紀錄 + 設定變更通知（管理員可設定） | `enhancement` | ✅ 2026-09-07 完成並上線 rev 00013-75s：audit_logs parent_entity 連結（worker 稽核帶 tenant_id + parent=bot，租戶變更紀錄併 worker 列與 entity_name）、設定變更通知（欄位群組 model / prompt / knowledge / tools / guard，租戶偏好端點，渠道 notify_config_change，AuditRecorder on_recorded fail-open 直接派送，平台變更也通知）；前端 worker 欄位對照、變更紀錄 worker 徽章、渠道開關、`/tenant-settings` 設定變更通知卡；bot_audit_visibility +5、config_change_notification 17 scenarios（26 cases）+ #75 guard_settings_api 8 個整合測試；unit 2008、前端 510 綠、mypy 195；migration 已套兩環境 |
| [#78](https://github.com/larry610881/agentic-rag-customer-service/issues/78) | OCR 多供應商：Gemini（OpenAI 相容視覺）OCR 引擎 + 依 KB.ocr_model / 租戶預設動態選引擎（現況只有 Claude 單例、KB.ocr_model 未生效） | `enhancement` | ✅ 2026-09-07 完成：OpenAI 相容視覺 OCR 引擎（google/openai/openrouter/litellm 共用、Gemini 走 json_schema 分類、抑制幻覺指令）、DynamicOcrEngineFactory（KB.ocr_model → 租戶 default_ocr_model → env `OCR_DEFAULT_MODEL`）、每文件獨立用量 tally（單例計數交叉汙染解除）、KB/租戶儲存驗證 provider、前端 KB/租戶 OCR 模型改 ModelSelect；ocr_engine_selection 15 scenarios，unit 1895、前端 422 綠、mypy 195；未部署、未打真 Gemini |
| [#79](https://github.com/larry610881/agentic-rag-customer-service/issues/79) | 廠商商品專有名詞對應表（OCR 後確定性校正），取代 prompt 內寫死品牌字；初始字典可用家樂福 865 商品 JSON | `enhancement` | 2026-09-08 開後 Larry 決定暫不做（無需求），已關為 not planned；OCR 紀律已改通用「專有名詞保護」（無寫死字樣） |
| ~~[#66](https://github.com/larry610881/agentic-rag-customer-service/issues/66)~~ | ~~需求三收尾：worker direct_retrieval 進 API/UI + bot mode profile~~ | ~~`enhancement`~~ | ~~需求書 §三~~ ✅ 2026-09-03（migration 已套 local-docker / company-poc-vm；POC rev 00010） |
| ~~[#58](https://github.com/larry610881/agentic-rag-customer-service/issues/58)~~ | ~~公開後台加固：登入失敗鎖定 + auth 端點節流 + LINE webhook 事件去重~~ | ~~`enhancement`~~ | ~~公司 POC 開放 unauth 後資安提醒~~ ✅ 2026-09-02 |
| [#67](https://github.com/larry610881/agentic-rag-customer-service/issues/67) | 認證面加固 P1–P5：register 邀請制、providers/mcp/workers 認證、租戶 API key（client_credentials）、JWT 改版（旋轉/撤銷）、widget 短效票、app_env 預設 production | `bug` | 3d-711 session 紅隊檢視 ✅ 2026-09-03 全部上線（migration 兩環境已套、POC rev 00011-x78、curl 驗收過） |
...
| [#96](https://github.com/larry610881/agentic-rag-customer-service/issues/96) | SSE 中斷漏記帳（M12）：記帳移入 `SendMessageUseCase`（三通路統一）+ 串流收尾包 `anyio.CancelScope(shield=True)` + 斷線 structured log | `bug` | **09-16 已上線 rev 00028-thr、Issue 已關**；線上量測前後皆 20/20 有記帳、14 天基準漏記 0.09%（09-08 六筆另查）——Cloud Run 不把客戶端斷線傳進容器，修復價值在三通路記帳路徑統一與結構性保護；生成中斷線的部分計費另案（需 migration） |
| API 契約審核第二塊（全 API 面） | 全 API 面 A1–F5 審核完成，報告 `docs/api-contract-review-2026-09-16.md`：11 項發現（Critical 1：29 個 date-time 欄位小數位不固定；High 5：無 4xx schema、207 處句子型錯誤、22 個建立型 POST 無冪等、9 個可空陣列、SSE 無 id/重連）+ 11 步改造順序（建議先做 1–6 約 3.5 天） | `review` | 09-16；順手修：errors.py dict-detail 回歸、widget 串流補 conversation_created（`_stream_events.py` 共用）；待 Larry 拍板改造順序 |
| [#97](https://github.com/larry610881/agentic-rag-customer-service/issues/97) | 契約改造第一批（1–6）：`ApiDateTime` 單一 profile（23 個回應欄位）、`ErrorResponse`/`ValidationErrorResponse` 進 OpenAPI（app 層 responses）、六支對外 router 65 處 `HTTPException` → `ApiError(code=…)`、9 個可空陣列的 null 語意寫進 schema、`scripts/export_openapi.py` + `docs/api/openapi.json` 快照測試 + `make openapi`、CORS expose 五個標頭 | `enhancement` | **09-16 已上線 rev 00030-k2j、抽查通過、Issue 已關**；抽查抓到 44 個宣告成 str 的 *_at 欄位一併改 ApiDateTime（共 67 個欄位）；main 待 Larry FF；7–11 待排 |
| [#98](https://github.com/larry610881/agentic-rag-customer-service/issues/98) | 契約改造第二批（7–11）：五支建立型端點 `Idempotency-Key`（含 multipart 上傳指紋，標頭進 OpenAPI）、`ApiMoney` 24 個金額欄位固定 6 位小數、無型別 object 標 `x-opaque` + 對話歷史 `structured_content` typed、`X-Client-Version` 中介層（`min_client_version`，低於門檻 426）、SSE `id:` 序號 + `docs/api/sse-contract.md` + widget 非串流 `POST /widget/{code}/chat` | `enhancement` | **09-16 已上線 rev agentic-rag-00031-2cr、抽查 8/8、Issue 已關**；12 個新 scenario；`Last-Event-ID` 重播與串流冪等同一觸發條件另案；11 步改造全部完成 |
| [#99](https://github.com/larry610881/agentic-rag-customer-service/issues/99) | 契約另案總收尾：串流冪等 + Last-Event-ID 重播、生成中斷線部分計費（estimated，**migration 待授權**）、usage 金額字串、全 router 143 處 ApiError、not_found 常數表、串流三段拆分、廠商交付包、VM 同步、channel-parity 二-2 trace 合一 / 二-3 攔截回應合一 / 二-6 LINE 接記憶 / 二-7 回放保真度標注 / 二-8 ADR-0003；GitHub 盤點關閉 10 個已完成 Issue | `enhancement` | **09-16 已上線 rev agentic-rag-00032-xfs、抽查 8/8、Issue 已關**；migration 兩環境已套並記錄；70 筆測試對話已刪；CI 的 oasdiff 留給 Azure DevOps 階段 |
...
| **LiteLLM 額度耗盡 → Anthropic 直連切換** | **✅ 完成** | **100%** | **Commits `759c1f1` data migration + `3978daa` 串流 list content fix。Data migration `migrate_litellm_to_anthropic_haiku.sql`：bots × 1 / tenants 5 default models / KBs × 2 ocr+context+classification / provider_settings disable litellm / guard_rules_configs default — 都從 `litellm:azure_ai/claude-haiku-4-5` 切到 `anthropic:claude-haiku-4-5`（同價同模型）。dev-vm + local-docker 都套並 verify，`_applied_migrations` 留紀錄。同時修 ReAct streaming 對 ChatAnthropic 直連回的 list[dict] content 處理（之前 `str(list)` 吃 Python repr 前端看到 `[{'text':...}]`）→ 加 7 regression tests** |
...
| **需求三收尾：bot mode profile（Issue #66）** | **✅ 完成** | **100%** | **2026-09-02：`Bot.mode`（fast/deep，快照白名單 + 指紋 extra）；fast = 無 worker 也走快速道、rerank/rewrite/HyDE 關、升級 ReAct 工具上限 2；deep = worker 快速道依設定（修正 #61 全域關 rerank）；worker CRUD API + 後台開關；bot 表單 mode 選擇；`FAST_LANE_ALLOW_RERANK` 移除；需求書 §三/§四 依現況更新。`fast_lane_profile.feature` 11 scenarios，unit 1472 全綠。前端 worker 開關 + 推理模式選擇（51 tests）；migration 已套 local / company-poc；POC rev 00010-2kn** |
| **security-precheck 首掃修復 + 依賴升級（2026-09-02）** | **✅ 完成** | **100%** | **公司 POC 首掃 688 請求：要修 17 → 修後 0。commit `153b276`：Pure ASGI SecurityHeadersMiddleware（HSTS/nosniff/XFO/CSP frame-ancestors/Referrer/Permissions、/api/ no-store）、production 關 docs/redoc/openapi、上傳 XML 改 defusedxml；POC `RATE_LIMIT_ENABLED=true`（auth 第 11 次 429 實測）。分支 `chore/deps-security-upgrade-2026-09`（`d3100bf`）pip-audit 全數修復（殘留 ecdsa 無修復版、HS256 不適用→已說明）；unit 1461 全綠、integration 升級前後同為 41 失敗（既有環境問題 → 新 Issue）。POC rev 00009 部署含升級版；VM worker 同步 uv sync。npm audit 34 筆皆 dev 工具鏈 → 已說明** |
| **快速道抽共用 + Gemini 對應（Issue #61）** | **⚠️ 程式完成，實測待驗** | **80%** | **2026-09-02：`DirectRetrievalService.plan()`（檢索 + DM 並行 + 門檻 + prompt 組裝 → plan，生成留各通路）；LINE `_try_direct_retrieval` 委派（六個既有 scenario 零改動）；web / widget 於 worker 命中 `direct_retrieval` 時走同一份（非串流回填 sources、串流補發 sources 事件）；快速道 profile：`retrieval_modes=["raw"]`、rerank 預設關（`fast_lane_allow_rerank`）；Gemini `normalize_reasoning_effort`（minimal→low）+ `reasoning_effort_allowed`（none/low/medium/high）。新 feature 12 scenarios，unit 全綠。待：Gemini 3.7 Flash low 20 題分流 / 12 題攻擊 / 10 題速度實測** |
| **設定指紋 + 稽核（Issue #60）** | **✅ 完成** | **100%** | **2026-09-02：`EffectiveConfig`（密鑰鍵名剝除、canonical JSON、sha256）+ `ConfigFingerprintService`（LRU + ON CONFLICT DO NOTHING、fail-open）；web / widget / LINE 於 prompt 組裝點打 `config_hash` 進 trace 與 usage（串流以內部事件傳 router）；`AuditRecorder`（diff-only、截斷、fail-open）掛進 guard 規則 / 平台 prompt / bot / worker ×3 / tenant 六個 use case，router 帶 actor，PUT bot 補版本作者；API `GET /config-snapshots/{hash}`、`/config-snapshots/diff`、`/bots/{id}/config-timeline`、`/audit-logs`；migration `add_config_snapshots_and_audit_logs.sql` + schema.sql 已同步。後端 18 scenarios，unit 1432 全綠。前端面板/時間軸/稽核頁見 commit** |
| **Trace 時間軸視圖（Issue #57）** | **✅ 完成** | **100%** | **2026-09-02：後端 `AgentTraceCollector.start(t0)` / `wrap_request()` / `span()` / `finish(None)`；web 補 conversation_load / bot_load / history_load / persist 節點、LINE 以 webhook 收到時為 t0 補 bot_load / webhook_verify / conversation_load / reply_push / persist（finish 搬到 reply 後）、RAG 拆 embed_query / vector_search 子節點；前端 `trace-timeline-layout.ts`（純函式，14 tests）+ `TraceTimeline` 元件（10 tests）接進 `agent-trace-detail` 的時間軸 tab（取代 recharts 長條）。後端 6 scenarios，unit 1414 全綠，`[no-migration]`** |
| **線上 LLM 自評下線 + 記帳補漏（Issue #59）** | **✅ 完成** | **100%** | **2026-09-02：移除每輪 `run_evaluation` enqueue 與 `_run_evaluations`/`_persist_eval`/diagnostic dispatch 死碼、worker 任務註冊移除、`eval_depth` 預設 off；`TRACE_THRESHOLD_MS` 同時閘控 console 與 `request_logs.trace_steps`（0=停用）；worker 新增每小時 `log_retention_cleanup_task`（`should_run_cleanup` 純函式）；query rewrite / HyDE 補 trace 節點 + usage（`account_aux_llm`）、記憶萃取修簽名 bug（原本靜默永遠回空）+ `usage_collector` 記帳、summary_recent 記帳（HistoryStrategyConfig.tenant_id）；對話摘要 cron 加 `conversation_summary_min_messages=6`。新 5 feature / 20 scenarios，unit 1408 全綠，`[no-migration]`** |
| **公開後台加固（Issue #58）** | **✅ 完成** | **100%** | **2026-09-02：`LoginAttemptTracker` port + Redis 實作（5 次失敗鎖 15 分鐘、成功清零、fail-open）→ login API 429 + Retry-After；rate limit 新增 `auth` 群組（login/register/token，fallback 10 rpm/IP）、client IP 改讀 X-Forwarded-For 尾段；LINE `webhookEventId` 去重（`WebhookEventDeduplicator` port + Redis SET NX EX 1h，驗簽後才去重，legacy `execute` 同步覆蓋）。新 3 feature / 28 scenarios，unit 1378 全綠，`[no-migration]`** |
| **認證面加固 P1（Issue #67）** | **✅ 完成** | **100%** | **2026-09-03：`/auth/register` 改邀請制——domain `registration_policy.can_register`（system_admin 任意；tenant_admin 只能在自己租戶建 user/tenant_admin；其餘 403、無憑證 401、未知角色 422）；`/auth/token` 非 development 回 404；`/settings/providers` CRUD/test-connection 僅 system_admin、enabled-models/model-registry 需登入；`/mcp-servers` 建/改/刪/discover/test-connection 僅 system_admin，list 非管理員一律以票內租戶查可用清單（忽略 query）、get 非管理員以 `McpServerRegistration.is_accessible_to` 過濾（不可用回 404）。`auth_surface_hardening.feature` 54 scenarios，unit 1526 全綠、auth/provider integration 25 全綠；integration/e2e 改用 admin 憑證。後續 P2 租戶 API key（client_credentials）、P3 JWT 改版、P4 widget token、P5 app_env 預設另開 Issue** |
| **認證面加固 P2–P5（Issue #67）** | **✅ 完成** | **100%** | **2026-09-03：P1b fence test（209 路由掃描；新發現 workers 四端點無租戶歸屬 + error-events 匿名）；P2 租戶 API key（`domain/auth/api_key`、sha256(salt+secret)、scopes/bot 範圍、撤銷 ver+1、`/auth/token` client_credentials、`/api-keys` CRUD + 前端「API 金鑰」頁、deps `authenticate`/`require_scope`）；P3 JWT（iss/aud/jti/kid、refresh family 旋轉 + 重用墓碑撤銷、users.token_version、改密碼即失效、Redis fail-open）；P4 widget 短效票（Origin 白名單必填、票綁 bot/Origin/visitor、伺服器簽發 visitor id、`?wt=` 文件檢視、widget 前端 session.ts）；P5 app_env 預設 production、租戶名稱登入移除、`docs/api-integration-guide.md`。unit 1609、integration auth/provider 28、widget vitest 15、frontend api-keys 7 全綠。**已上線**：migration 兩環境已套、rc-a6a0299 → rev 00011-x78、main 68667d9、VM worker 重啟、curl 驗收通過；未做：auth 群組 client_id 速率維度（middleware 讀不到 body，IP 維度已涵蓋）、`.env.example` 更新（檔案受權限保護）** |
| **異常使用者分級控管 P7a（Issue #68）** | **✅ 程式完成，待 migration + 部署** | **P7a 100%（P7b–d 待做）** | **2026-09-03：`domain/abuse`（主體 / 訊號 / 政策 / 分級決定）、`AbuseControlService`（evaluate / record / release，fail-open，audit）、Redis Lua 衰減分數 + 記憶體版；send_message 與 LINE 兩條管線同一 service（L1 保守模式、L2 固定文案、L3+ 拒絕→429 `temporarily_unavailable`）；rate limiter L2 主體 5 次/分；widget Origin 不符計分；trace `abuse_level`（migration `add_trace_abuse_level.sql` 待授權）；`docs/abuse-control-behavior.md`。abuse_scoring 13 + abuse_pipeline 13 scenarios，unit 1635 綠** |
| **異常控管告警 P7c-通知（Issue #68）** | **✅ 程式完成，待 migration + 部署** | **P7c 通知 100%（設定三層 / 後台頁 / 監控模式租戶級 待做）** | **2026-09-04：Teams Workflows Adaptive Card 發送器（舊 Incoming Webhook 不支援）、AbuseAlertService（L3+ 即時、fail-open 必發 + 15 分冷卻、429/L2 每租戶 5 分鐘達 20 次發一次）、每日摘要 cron 09:15 台北、通知內容遮罩主體 id；NotificationDispatcher 解密設定 + 單渠道失敗隔離（修 email 讀密文的舊 bug）；`notification_channels.notify_abuse`（migration 待授權）；abuse_alerts.feature 9 scenarios；unit 1644 綠** |
| **異常控管設定三層 P7c-設定（Issue #68）** | **✅ 前後端完成，待 migration + 部署** | **100%** | **2026-09-04 前端 1fc7e44：`/admin/abuse-control` 四分頁（系統預設 / 方案 / 租戶設定 / 受控中，僅 system_admin 可寫、覆寫表單只送有設定的鍵、有效值含來源標記）+ `/abuse-status` 租戶唯讀（遮罩主體、無解除）；10 個單元測試，前端 355 綠。後端 2026-09-04（Larry 定案：系統預設 + 方案 + 每租戶指定，只有系統租戶能改）：`domain/abuse/settings`（platform / profile / tenant 三 scope 覆寫 JSON、平台硬上限 BOUNDS、門檻遞增檢查、內建方案 standard/strict/lenient/monitor）、`CachedAbusePolicyProvider`（每租戶 60 秒快取、DB 失效退回預設）、Update 用例（驗方案、稽核、清快取）、受控清單 / 解除；`/api/v1/admin/abuse/*` 寫入僅 system_admin、tenant_admin 只讀自己；`abuse_settings` 表（migration `add_abuse_settings.sql` 待授權）；abuse_settings.feature 28 scenarios，unit 1672 綠** |
| **異常控管聚合層 P7d（Issue #68）** | **✅ 完成** | **100%** | **2026-09-04：主體達 L3 → `aggregate_weight` 12 加到 IP 與租戶聚合層；IP 達 30 → L4 封鎖 1 小時（新主體一律拒絕，白名單 / 可關）；租戶達 30 → L1 全租戶保守模式 + rate limiter 上限減半 + 「疑似受攻擊」Teams 告警（永不拒絕）；routers 帶 client_ip（X-Forwarded-For 尾段）；abuse_aggregation.feature 6 scenarios；unit 1678 綠** |
| **異常控管通路補強 P7b（Issue #68）** | **✅ 全部完成，待部署** | **100%** | **2026-09-04 前端 a19dcf2：`/widget-identity` 設定頁（secret 輪替只顯示一次、啟用 / 強制驗證開關、宿主整合說明四語言範例；tenant_admin 管自己、system_admin 選租戶）；6 個單元測試，前端 361 綠。後端 2026-09-04：widget 簽發端點 `widget_issue` 群組 30/分/IP；宿主身分綁定 identify()（`domain/widget/identity` HMAC-SHA256(secret, user_id.exp)、租戶 secret 加密存放可輪替 / 停用 / 強制驗證、`POST /widget/{code}/identify` 換帶 end_user_id 的票、失敗 +2、`window.AgenticRagWidget.identify()`、`docs/widget-identity-protocol.md` 四語言範例）；LINE 群組總量超標只計洗版者 + 群組內 L2+ 靜默；`tenant_identity_secrets` 表（migration 待授權）；widget_identity.feature 17 scenarios；unit 1694 綠。挑戰驗證 → #69 待主管討論** |
| **P7 測試前待辦（Larry）** | **⏳ 待 P7d 完成後一次測** | **—** | **① migration：api_keys / users.token_version / trace abuse_level / notification notify_abuse 四支已套 local-docker + company-poc-vm ✅；② 後台通知渠道填 Teams Workflows webhook URL 並按「測試發送」確認 Adaptive Card；③ Email 暫不設定，確認顯示「未設定」不報錯；④ `docker push` 已推 rc-935df5d，`gcloud run deploy` 待 Larry 執行（POC 現仍 rev 00011-x78）；部署後 curl：providers / mcp-servers 無憑證 401、/auth/token 舊語意 400、client_credentials 換票打 /agent/chat 200、L3 主體 429 temporarily_unavailable；⑤ 建 WebView 用 API key（chat:send，綁看板 + 入口 bot）安全管道交 3d-711；⑥ 新租戶先 monitor 一週再切 enforce，觀察 fail-open / 429 告警是否進 Teams** |
...
| **Bot-level RAG retrieval modes — multi-mode + Playground 合流** | **✅ 完成（待 dev-vm migration + CF Pages 部署）** | **100% 程式碼** | **Issue [#43](https://github.com/larry610881/agentic-rag-customer-service/issues/43): Stage 2.1-2.7 完成。(2.1) Domain RetrievalMode enum + Bot entity 加 7 欄位 + bot_model + bot_repository + create/update use case + bot_router schema + migration `add_bot_rag_modes.sql`（local-docker 套用 + 寫 _applied_migrations）. (2.2) `_hyde_generator.py` + QueryRAGUseCase multi-mode retrieve（並行展開 N 條 query → embed → search × kbs → union by chunk_id 保留最高分 → rerank → top_k）+ extra_hint 支援. (2.3) RAGQueryTool / react_agent_service / send_message_use_case 一路傳 metadata. (2.4) Bot detail UI 新 RetrievalModesSection（3 checkbox + 各 mode model + extra_hint textarea + zod min(1)）. (2.5) Retrieval Playground 改 multi-mode select + 顯示 mode_queries + 「✓ 套用到 Bot」按鈕（下拉選 bot 一鍵 useUpdateBot）. (2.6) `test_retrieval_use_case` 改成 `query_rag_use_case` thin wrapper — Playground 跟 real RAG 100% 同程式路徑. (2.7) 6 BDD scenarios（multi_mode_retrieval.feature）+ 858 unit tests + 15 bot integration tests + 46 frontend tests + frontend build 全綠. Architecture journal 已寫. ⚠️ TODO: dev-vm migration（push 前必做）+ Cloud Run deploy 後驗證 trace metadata 帶 modes/mode_queries** |
| **External Producer Integration — Source Tracking + Bulk Ingest + Unified Search** | **✅ 完成（待 Milvus 2.6 升級才 backfill 既有 collection）** | **100% 程式碼 + DDL 已套** | **Issue [#44](https://github.com/larry610881/agentic-rag-customer-service/issues/44) 完整三 phase. **Phase 1** (commit `1d54fcb`): Milvus first-class source/source_id 欄位 + INVERTED index + `_build_filter_expr` IN operator + `DELETE /knowledge-bases/{kb_id}/documents/by-source` endpoint + tenant 雙重防線. **Phase 2** (commit `c44f9f5` 部分): Document/ORM 加 source/source_id 欄位 + DB migration `add_document_source_tracking.sql` (local-docker + dev-vm 已套用) + `BulkIngestUseCase` + `POST /documents/bulk`（≤100 筆，partial response，重 push 自動 dedup via Phase 1 endpoint）+ `process_document` propagate source 到 Milvus chunk payload. **Phase 3** (commit `c44f9f5` 部分): `QueryRAGCommand.extra_filters` + `UnifiedSearchUseCase`（thin wrapper of QueryRAGUseCase）+ `POST /api/v1/rag/search` 跨多 KB + multi-mode + filter + rerank. **Resilience fix** (commit `909f405`): `MilvusVectorStore.upsert/search` 自動偵測 collection schema 並過濾未宣告欄位 — Milvus 2.5.6 server 沒 `AddCollectionField` RPC 也不擋 ingest（既有 collection 沒 source/source_id 也安全；新 collection 自動拿到完整 schema）. 12 BDD scenarios + 10 unit tests + 868 既有 unit tests 全綠. Architecture journal Phase 1 段已寫. ⚠️ Backlog: 升級 Milvus 2.6 後再跑 `scripts/add_source_fields_to_milvus.py` 把 2 個既有 collection backfill。** |
| **康達盛通 POC 反饋修復（P0+P1）** | **🔄 進行中** | **90%** | **Plan `.claude/plans/poc-feedback-fix-plan.md`。8 題根因全數線上 trace 實證（trace 30 筆 + Milvus 逐鍵比對）。WP-A 問題7：dm_image_query_tool context 改用去重前結果組裝（包大人同頁被 by_doc dedup 砍出 context）+ 2 regression + BDD feature。WP-H 問題2：flex_contact_builder 按鈕 URL 加 openExternalBrowser=1（Android LINE WebView 不彈授權致網路電話卡住）+ 5 tests。WP-B2 問題4：LINE 出口 strip_markdown_for_line 安全網 + 9 tests + prompt_defaults seed 加格式/連結規則。1049 unit tests pass。P1 完成：WP-B1/C SQL 已套 dev-vm + _applied_migrations（9 rows verify ✓）。WP-D3 查詢異體字正規化（週→周+NFKC，只碰 embedding 輸入）。WP-F1/F2 guard 去重（_input_guard_checked 協商標記，decorator 契約不變）+ LINE guard∥intent 並行（Larry 核可風險）。WP-E 重爬完成：官方站已改版 uni-prosperity.com.tw（萬家福），Edge headless dump 16 分類 121 題（實名制 14 題、理髮店 81 家）→ scripts/uni-prosperity-faq-data.json。1058 unit tests pass。D1/D2 SQL 內容已在 dev-vm 生效並唯讀驗證 ✓（觸發詞+雙 KB 綁定），唯 _applied_migrations 記錄待 Larry 回覆「保留」補寫。🔧 追加修復：LINE 載入動畫從未顯示（端點少 /start → 404 靜默失效，Larry 實測發現）— 修正 URL + 2 regression tests。② FAQ 重匯 ✅ 完成（Larry 後台上傳 uni-prosperity-faq-data.json → 121 chunks processed；Postgres+Milvus 雙端驗證：實名 14 筆、81間理髮含板橋、舊 86間 0 殘留）。⏳ 待 Larry：① D1/D2 保留/退回、③ 部署 ✅ 完成（Larry 授權手動部署：docker build e285295 → Artifact Registry → Cloud Run revision agentic-rag-00286-ktf 100% 流量，/api/v1/health 200）。✅ push 已完成（2026-07-21 確認 local/origin 同步）。完成後 Android 實機驗證 + 8 題重測** |
| **LINE Webhook 延遲優化 R1（回覆先行 + reasoning_effort 接通）** | **🔄 進行中** | **80%** | **Issue [#49](https://github.com/larry610881/agentic-rag-customer-service/issues/49)。實測基準：traces 52 筆 p50 8.4s / p90 11.7s，瓶頸=三次串行 LLM round-trip（意圖分類 ~1.4s → ReAct 迭代1 ~2s → 迭代2 生成 ~3s），向量搜尋僅 13-49ms。**已完成**：(1) `_process_single_event` 回覆先行 — reply → finally 持久化（存對話/trace/usage 後移），timing log 拆 `persist_ms`；(2) `show_loading` 改 fire-and-forget；(3) **接通 `reasoning_effort` 死設定**（bots 欄位存在但聊天路徑從未傳遞 → gpt-5.4 一直跑 medium reasoning）：webhook llm_params → react_agent → ChatOpenAI + OpenAILLMService.get_chat_model，`supports_reasoning_effort` gate 只對 gpt-5/o-series 夾帶，僅 LINE 通路傳遞 widget 不受影響。3 BDD scenarios + 5 unit tests，LINE 套件 72/72、全量 unit 1068 pass，ruff/mypy 零新增。**DB 已套**（dev-vm UPDATE 1 verify ✓，Larry 確認）：bot 改名「LINE 專用客服（家樂福 POC）」+ reasoning_effort=low + history_limit=6（不動 max_tokens — 上限不影響速度）。部署 ✅ revision 00289-x9j（health 200）。⚠️ **部署後實證發現 API 限制**：gpt-5.4 + function tools 在 chat completions 只接受 reasoning_effort='none'（400 拒絕 low/medium/high）→ 設 low 導致 agent 呼叫全滅、LINE 無回應。🔧 hotfix：`reasoning_effort_allowed` gate — gpt-5 系列 tools 場景只放行 'none'，其他值略過不傳（回到 provider 預設）+ warning log，寧可慢不可斷；4 tests 改寫/新增，1070 unit pass。DB 可維持 low 不再 400。hotfix 部署 ✅ revision 00290-2g5（100% 流量，health 200），Larry LINE 實測恢復 ✓（首則 15.8s 為換版冷啟動：agent 前置 4.7s + Milvus 首連 4.1s，非新常態）。**dev-vm 設定 UPDATE 紀錄（皆 Larry 逐句授權，phase=dev DML）**：2026-07-21 ① `UPDATE bots SET name='LINE 專用客服（家樂福 POC）', reasoning_effort='low', history_limit=6 WHERE id=2feba9a0`（UPDATE 1 verify ✓，low 後因 API 限制由 gate 略過）② `UPDATE bots SET reasoning_effort='none' WHERE id=2feba9a0`（UPDATE 1 verify ✓，關閉推理拿主要提速，Larry 確認）。⏳ 待 Larry：熱機後 LINE 實測 2~3 簡單題（速度）+ 8 題複雜題抽測（品質回歸）→ 產 before/after 對比表；若品質退步一句 SQL 退回 ''（回預設 medium）。新增隱憂（優先級高）：LLM 全滅時 LINE 無聲失敗，缺 catch-all 錯誤回覆兜底。**R2 模型分級（2026-07-21，Larry 逐句授權）**：實測發現 gpt-5.4 延遲主因是輸入處理+基礎服務速度而非推理（'none' 僅省 ~0.5s）→ 換模型是最大槓桿。`UPDATE bot_workers SET llm_provider='anthropic', llm_model='claude-haiku-4-5' WHERE name IN (商品查詢,門市服務查詢,閒聊)`（UPDATE 3 verify ✓，走既有 anthropic 直連零部署）；高階客服維持 gpt-5.4。預期 FAQ 類 p50 → ~4-5s。⤴️ **2026-07-21 Larry 指示還原**：3 個 worker 覆寫清空（UPDATE 3 verify ✓），全部回到 bot 預設 gpt-5.4（reasoning='none' 維持）；Haiku 分級首測 7.1s 僅 1 筆，日後要重試可再下同樣的 UPDATE。**R2.5 TTFT 儀表**：串流路徑加 `first_token` trace 節點（web 首字時間，量化 web vs LINE 體感差）+ 1 test，1071 unit pass。**交付物**：`LINE延遲分析_2026-07-21.xlsx`（62 筆逐階段拆解+圖表，已交 Larry 桌面）；重產工具入庫 `scripts/latency_report/`（README 含步驟）— ⏳ 待補 Haiku 樣本+TTFT 數據後重產更新。**R2.6 前置斷點儀表（2026-07-23，PM 要求逐斷點計時）**：collector 提前至 webhook t0 啟動 + history_load/input_guard 節點；發現既有 intent_classify 節點因 collector 晚啟動兩週未生效（fail-open no-op），此改一併救活。1 test，1072 unit pass，deploy 後 LINE trace 斷點 100% 覆蓋。PM 報告已交（給PM的結論分頁 + OPENPOINT 對比分析：App 內串流 vs LINE 無串流為主因）。**方案A 直接檢索模式 ✅ 實作完成（Issue [#50](https://github.com/larry610881/agentic-rag-customer-service/issues/50)，2026-07-23）**：bot_workers.direct_retrieval 開關（migration 已套 dev-vm + _applied_migrations ✓；infra/schema.sql 自 4 月誤刪後重建）；快速道=分流即檢索（重用 QueryRAGUseCase）→ 門檻判定 → enabled_tools=[] 單次生成；未過門檻/異常自動升級 ReAct + escalated trace 節點。4 BDD scenarios，1076 unit pass，預設 false 零行為變更。部署 ✅（00293~00295）+ 兩 worker 已開啟（UPDATE 2 ✓，Larry 確認）。**第一輪實測達標**：11 題快速道中位數 5.2s（基準 8.3s，-37%）、升級率 0%、B組不受影響。實測兩修：①DM 圖卡消失 → 並行呼叫 DM 工具本體（signed URL 非向量 payload）②工具名稱裸吐 → 快速道保留 transfer_to_human_agent 唯一工具 + prompt 禁裸吐。**✅ 結案（2026-07-23 Larry 確認）**：品質/圖卡人工確認 OK；剩餘 7 題同質判定風險低不補測；Issue #49/#50 均關閉。最終成果：FAQ/商品題 P50 8.3s→5.3s（-37%）、升級率 0%、LLM 呼叫 2→1；Excel 最終版（含給PM結論+快速道P4數據）已交付+PM 文案已提供。後續 backlog：意圖分類提速/一致性（進 4 秒級）、FAQ 內容缺口補強（門市總數等）、web TTFT 數據（widget 尚未實測）。其餘 backlog：RAG prefetch（∥迭代1，省 ~1s）、router 提速、輸入瘦身、FAQ 語意快取、Anthropic prompt caching（agent system prompt）、p90>10s LINE retry 去重檢查** |
| **LINE 延遲優化 R3 — 非 LLM 段（預取/快取/router 降級）** | **✅ 驗收完成（avg 3.8s，-51%）** | **100%** | **Issue [#52](https://github.com/larry610881/agentic-rag-customer-service/issues/52)。基準：dev-vm traces 近 30 天 gpt-5.4 108 筆 avg 7.8s = LLM 3.9s + 非 LLM 3.9s（guard∥意圖分類 1.77s、檢索 1.2s、設定 DB 查詢）。**E1 Embedding 查詢快取**：`CachedEmbeddingService` decorator（Redis，key=emb:q:{sha256(model+text)}，float32 base64，TTL 7d，fail-open），wire 進 container 全 embed_query 受益，命中省 ~300-500ms。**E2 設定快取**：GuardRulesConfig（含 None 快取）/ WorkerConfig 60s module-level TTLCache（比照 dynamic_llm_factory 前例），寫入全清失效。**E3 檢索預取並行**：快速道檢索與 guard∥意圖分類同時起跑（原文 + bot 預設 KB），分類後查詢被改寫（#51 follow-up）或 worker 覆寫 KB → 丟棄重查；guard 攔截/ReAct → 取消；預取失敗 fail-open 回退正常檢索；trace 節點加 prefetched 欄位。**E4 router 降級**：`UPDATE bots SET router_model='openai:gpt-5-nano' WHERE id=2feba9a0`（UPDATE 1 verify ✓ + _applied_migrations 紀錄，Larry 授權），意圖分類段預期 1.77s → ~0.6s。BDD 26 scenarios + 全量 unit 1107 pass。Commits `6464955`（E1-E3）+ `2399960`（E4 migration 檔）。**E4 三輪實證後判定不可行並回滾**：nano 拒收 temperature=0 與 reasoning_effort='none'（各觸發一輪修復：learn-and-strip 剝參數重試 `149f4a0`、reasoning_effort hint `9f3ffb9`、minimal+分類空輸出安全網 `a85102a`），reasoning 燒光 400 tokens 預算輸出空、耗時 2.8-4.3s 反比 gpt-5.4 的 1.7s 慢 → `issue52_router_model_revert.sql` 退回（UPDATE 1 ✓ + _applied_migrations）；三輪硬化程式碼保留（routing 不再可能靜默全滅）。**E5 驗收 ✅（2026-07-29 第四輪實測）**：5 題全套生效 — 分流 5/5 正確 0 fallback、快速道單次 LLM 0 升級、#51 改寫命中（「價格呢」→含商品名）、embedding 快取 hit 2-9ms vs miss 570-1040ms、重問題檢索 22ms。**排除冷啟動 avg 3.8s（基準 7.8s，-51%）**，Larry 體感確認變快。已知限制：檢索預取未採用（兩個快速道 worker 各綁單一 KB 與 bot 預設不同 → 推測失效，損失已被 embedding 快取補償）→ backlog：per-worker 預取。模型更換（Gemini Flash 對標，預估 LLM 段再 -1.5~2s）另案評估** |
| **LINE 回應瘦身 R4 — 目標 ≤5s（砍輸出重量 / 高階客服快速道 / 關 LLM input guard）** | **🔄 進行中** | **40%** | **2026-08-17 POC 環境重啟**（VM + `db-connector` 重建 + min-instances=1，rev 00304-pf5 = ac364a8）。**最新實測**：R3 驗收後 07-30/31 真實使用 12 筆 avg 6.9s、≤5s 僅 1 筆；07-29~31 共 33 筆 avg 6.6s。拆解：input_guard（LLM haiku）avg 2.1s 已超過意圖分類 1.46s 成前置瓶頸；高階客服 ReAct 2 次 LLM 4.2-5.1s 五筆全部 6.8-10.5s；輸出 125-279 tok（gpt-5.4 ~51 tok/s → 生成段 2.5-5s）= 「回應太重」。**方案**（全部 DML、60s 快取生效免部署）：① max_tokens 2048/1024/1024/512 → 450/400/300/150 + worker prompt 追加「回覆長度」規範（≤120 字、條列 ≤3；門市清單例外保留）② 高階客服 `direct_retrieval=true`（LLM 2→1，transfer 保留）③ `guard_rules_configs.llm_input_guard_enabled=false`（保留 25 條規則）。migration 檔 `apps/backend/migrations/line_response_slimming_2026-08-17.sql` 已寫（含 _applied_migrations 紀錄）。DML 已套 dev-vm ✅（UPDATE 1/1/1/1/3/1/1/1 verify ✓，commit `425d6a8`）。**🐛 實測踩到既有 bug（16:21「機車中心呢」answer_len=0 → LINE 400 無聲失敗）**：快速道 `max_tool_calls=1` 但 `should_continue` 計的是 LLM 迭代數 → 第 1 輪 tool_call 就 END、transfer 從未執行、回答為空；reply API 400 又不記 log。修復：①`should_continue` 改 `call_count - 1 >= max_tool_calls`（語意=最多執行幾輪工具）②LINE 空回覆兜底句（有聯絡卡→轉真人引導；否則→換問法/真人客服）③`line.reply.failed` warning。2 BDD regression scenarios，1123 unit pass，ruff/mypy 零新增。部署後實測（rev 00305/00306）：**avg 3.9s（排冷啟 3.6s）、≤5s 8/10，三方最快**（OPENPOINT 5.5s / 萬家福官方 LINE 5.0s）；攻擊 9 題守 6 🟡2（改寫式軟洩漏）🔴1（七言絕句文體劫持）；再修快速道漏傳 customer_service_url（`34d4de5`，轉真人有字沒按鈕）。**R5 防護減法 + 前置語意閘門（2026-08-17 下午）**：①`48d0e5c` 減法 — 移除 E3 預取（永遠失效、多一次 embedding）、移除 LLM input/output guard 程式與後台 UI 區塊（DB 欄位保留不生效）、`LINE_CHANNEL_PROMPT_SUFFIX` 把輸出格式/回覆長度/角色鎖由 webhook 注入一次取代 5 份重複 prompt、regex 預設去角色扮演類（DB 25→17，`guard_slimming_2026-08-17.sql` ✅ 已套 dev-vm 09:25Z：UPDATE 4/1/1/1 verify ✓，prompt 重複區塊全清）；②分類器三行協定 `classify_sanitize`（類別 / 清洗+改寫查詢 / ATTACK|OK）— 純攻擊 → `PromptGuardService.block_by_classifier` 回同一份 blocked_response 固定文案、不進檢索與生成（拒答 ≈ 分類耗時 ~1.5s，攻擊句不進主模型）；混合型（小新語氣+問點數）剝語氣留問題。3 unit + 1 BDD；1117 unit pass。✅ 部署 rev 00307-b9v + DML 已套；⏳ Larry 攻擊 12 題 + 速度 10 題重測。**單 agent 盤點結論**：worker 切分本身 ≈ 0 成本（設定快取），分類那次 LLM 是為了攻擊判定與 follow-up 改寫，拿掉 worker 也拿不掉；下一步若要再省是分類降本（embedding 路由，C 案）** |
