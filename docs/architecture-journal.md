...
## 金鑰輪替 — 密文自帶版本、相容舊格式、用圍籬守住「重新加密的範圍」（2026-09-23，Issue #105）

**Sprint 來源**：infra 要把四個敏感 env 改成 Secret Manager 掛載，Larry 決定先把加密金鑰輪替機制做好。
原本 `AESEncryptionService` 只有一把金鑰、密文不帶版本，換金鑰等於資料全部解不開。

**主題**：Envelope-less key rotation、向後相容的資料格式、維運腳本的涵蓋範圍守門

#### 做得好的地方
- **密文自帶 key id，解密依前綴選金鑰**：`<key_id>:<base64>`，無前綴一律視為 `v1`。輪替分成
  「加入新金鑰 → 切換 active → 重新加密 → 移除舊金鑰」四步，每一步都可獨立驗證、可回滾。
- **active 為 `v1` 時照舊寫無前綴格式**：規格同時要求「新格式」與「只設舊變數時行為完全相同」，
  兩者衝突時選擇後者優先——這讓 B9 剛做好的管線回滾在第一次輪替前完全安全。代價是格式有兩種，
  由 `split_ciphertext` 一處吸收。
- **設定錯誤在啟動時就失敗**：hex 長度、id 格式、重複 id、active 出現在 previous 都拒絕啟動，
  不退回預設金鑰；錯誤訊息只有 id 與長度，不含金鑰內容。
- **圍籬測試守住「重新加密腳本涵蓋所有密文欄位」**：AST 掃 `src/` 每個 `.encrypt(` 呼叫點，
  必須登記為 DB 欄位或「只寫 Redis 快取」並附理由。這是最容易在半年後悄悄壞掉的地方——
  新增加密欄位卻忘了進腳本，要等到移除舊金鑰那天才發現資料解不開。

#### 潛在隱憂
- **步驟 2 之後不可把應用程式回滾到 #105 之前**：舊版不認得 `v2:` 前綴。runbook 已寫明，但管線 3888 的
  rollback 不會擋。→ 改善：rollback 解析目標版本時，若 `ENCRYPTION_MASTER_KEY_ID != v1` 且目標 commit
  早於 #105 則紅燈 → **優先級：中**
- **LINE 憑證在 DB 是明文**（at-rest 加密另案），不在輪替範圍，也是四個敏感值之外最大的缺口
  → **優先級：高**（另開 Issue）
- **沒有 KMS / envelope encryption**：資料金鑰就是主金鑰，輪替要重寫所有密文。資料量大時改成
  per-record data key + KMS 包裝，輪替只需重包 data key → **優先級：低**（目前只有個位數筆）

#### 延伸學習
- **Envelope encryption**（Google Cloud KMS / AWS KMS 的標準做法）：為什麼大規模系統輪替金鑰時不用重寫資料。
- **Crypto agility**：密文帶演算法與金鑰版本，讓「換演算法」和「換金鑰」一樣是設定變更。
- **討論題**：重新加密腳本目前是手動執行的維運步驟。若改成 worker 背景作業自動做，要怎麼避免
  「還有執行者不認得新金鑰時就開始寫新密文」這個順序問題？
...
「等待中」轉圈圈。使用者的原話是：「我不要這種系統假的、其實是 bug 的內容。」

**主題**：重試粒度、有副作用的步驟、狀態誠實化、佇列可觀測性

#### 做得好的地方
- **用資料本身而不是猜測定位根因**。三份 pending 各自都有一份**同檔名、晚 1.6–2.4 秒
  建立的 processed 雙胞胎**，而 worker log 近 30 分鐘有 496 次 `process_document`、
  grep 這三個 document_id 零筆命中。兩個事實合起來只有一種解釋：文件列建立了兩次，
  第一次那列從來沒被派工。時間差 ~2 秒正好是一輪 429 重試。
- **修在粒度，不是修在症狀**。上傳是三步（request-upload 建立文件列 → 直傳 GCS →
  confirm-upload 派工），而重試的粒度是**整個三步**。錯不在限流、也不在佇列，錯在
  「重試把一個有副作用的步驟又做了一次」。改成佇列保管一個跨重試共享的 `resume`
  物件，任務自己記住哪幾步做完了——**佇列不需要知道上傳分幾步**，抽象邊界保住了。
- **安全網用「佇列見底」而不是「等太久」當判準**。worker 一撿到工作就把文件轉
  `processing`，所以「還是 pending」+「arq 佇列長度為 0」= 這份工作根本不存在，可以
  直接判死。純看年齡會誤殺 `split_pdf` 生出的上百個合法排隊 child（單 worker
  `max_jobs=3` 跑完要半小時以上）。佇列探測失敗回 `-1`，走保守分支而不是誤判。
- **順手抓到同一類的第二個謊**。`retryFailed()` 只把徽章改回「排隊中」，從來沒有把工作
  放回 `pendingRef`、也沒有 `pump()`——按鈕看起來有反應，實際上永遠不會送出。
  跟「等待中」一模一樣的假狀態，一起修掉。

#### 潛在隱憂
- **`enqueue()` 吞掉所有例外只回 `None`，呼叫端沒有一處檢查回傳值**。這次的 reaper 只是
  安全網；真正該做的是讓 `confirm-upload` 在派工失敗時就回 5xx，讓前端當場知道。
  現在的設計是「先騙使用者、五分鐘後由 cron 說實話」→ 優先級：中。
- **簽名網址 600 秒效期沒有續期路徑**。限流暫停累積超過十分鐘時，resume 會拿著過期網址
  PUT 得到 403。目前的結果是該檔顯示真實失敗（可接受），但更好的做法是提供
  「以既有 document_id 重新取得簽名網址」的端點，才能真正做到零孤兒 → 優先級：低。
- **`find_stale_pending` 沒有租戶過濾**。它是平台級的維運掃描（跨租戶），語意上正確，
  但這是專案裡少數刻意不帶 `tenant_id` 的查詢；未來若被誤用在租戶端 API 就是隔離破口。
  已用命名（`stale_pending`）與註解標示用途，仍建議日後加上「僅限 worker 呼叫」的護欄
  → 優先級：中。

#### 延伸學習
- **Idempotency key**：這次的根本問題是 `request-upload` 不冪等。若它接受一個由前端產生的
  冪等鍵（檔名+大小+lastModified 的雜湊），重試自然落回同一列文件，連 `resume` 都不需要。
  這是比「記住做到哪一步」更強的解法，代價是後端要存鍵。搜尋關鍵字：
  `idempotency key`、`exactly-once vs effectively-once`。
- **Saga / 補償交易**：三步流程中任一步失敗都要有明確的補償動作（這次缺的是「第一步成功
  但第三步失敗」的補償）。reaper 本質上就是一個延遲執行的補償交易。
- **思考題**：如果 `confirm-upload` 改成派工失敗即回 5xx，前端重試又會落到同一列文件上
  （因為有 resume），此時 `enqueue` 若其實成功了只是回應遺失，就會派兩次工。
  你要在哪一層做去重——arq 的 `_job_id`、`processing_tasks` 的唯一鍵、還是
  `process_document` 開頭的狀態檢查？三個答案各自的失效模式是什麼？

---

## 量測工具本身會說謊 — 三個靜默失真與兩個供應商契約破口（2026-09-08，Issue #83/#84/#85）

**Sprint 來源**：地端模型評測支線。原本只是要跑題比較五個模型，結果一天內在「量測工具」
與「產品契約」兩側各挖出三個問題，而且全部都是**不報錯的那種**。

**主題**：靜默失真、供應商相容層的抽象洩漏、契約一致性、評測方法學

#### 做得好的地方
- **用「五個模型結果一模一樣」當偵測訊號**。JSON 題組首跑三個不同供應商的分數完全相同
  （92%/92%/83%），這在機率上不合理——查下去是防護攔截的固定話術，輸出根本沒經過模型
  （105ms、純文字）。同樣的手法也抓到 60 輪那批「五臂在同幾題一起漏同幾個數字」，
  真因是跑題器沒接上下文。**跨臂一致 = 平台在說話，不是模型在說話**，這個訊號很好用。
- **把「不可信的量測」當 bug 修，而不是當雜訊容忍**。SSE 的 `retrieval` 事件在結尾才發、
  `conversation_id` 在第一個 `done` 之後才發，兩者都讓我先前得出過錯誤結論（「檢索佔首
  token 七成」「Gemini 翻動率 61%」）並寫進對話。修完後改用伺服器端 trace 的
  `start_ms/end_ms` 重算，答案完全相反：前置管線約 60ms、模型佔 95–99%。
- **靜默退化一律改成明講**。`analyze_runs.py` 原本在 tiktoken 缺席時退回字元數，讓「字元/秒」
  與「token/秒」變成同一個數字卻看起來像兩個指標；改成略過該欄並提示。跑題器拿不到
  `conversation_id` 時直接印警告。**寧可少一欄，不要假的一欄。**
- **供應商能力差異要在「存檔階段」擋，不是在「使用者面前」炸**。Gemini 綁工具必定 400
  （thought_signature 被 langchain-openai 雙向剝除），先加 `TOOL_UNSUPPORTED_PROVIDERS`
  在 bot 存檔時回 422；真修法（原生 SDK）完成後把清單清空、機制保留。
- **原生 SDK 只切在「綁工具」那條路**。`_resolve_llm_model(..., with_tools=bool(tools))`，
  kb 模式維持相容端點。理由不是技術而是**評測可比性**：整條換掉會讓當天已收的 Gemini
  基準失效。這是「為了不破壞既有量測而刻意保留兩條路」的取捨，值得記下來。

#### 潛在隱憂
- **兩條 Gemini 路徑是維護債**。同一個供應商掛兩套 SDK，意味著兩套 usage 擷取、兩套推理
  強度對應（`reasoning_effort` vs `thinking_budget`）、兩種失敗模式。評測結束後應該統一，
  但統一前要先做 A/B（同 10 題各走一次）確認差異幅度 → 優先級：中。
- **`langchain-openai` 剝除供應商專屬欄位是結構性風險**。今天是 Gemini 的
  `thought_signature`，明天可能是別家的別的欄位。相容端點本質上是最小公倍數，
  只要供應商往「有狀態的工具往返」走就會踩到 → 值得在 ADR 記一筆：**新供應商接工具前
  先驗一次「工具回合後的第二次呼叫」**，不要只驗單次生成。
- **混合 OCR 的合併沒有真的執行**（#83）。落庫文字沒有經過 `render_ocr_text` 正規化，
  92 個商品條目只有 74 個唯一、8 個欄位互相矛盾。後果不是報錯而是**同一題兩個模型各有所本、
  無法判對錯**。根因未定位，`ocr_merge` 本身測起來是對的，問題在呼叫端 → 優先級：高。
- **評測題組的金標準會反噬**。機械篩選一度顯示五臂都在 M3-1 漏了「7/1」「6/30」，實際上
  模型寫的是「7月1日至隔年6月30日」——是我的比對器只認字面。金標準與比對器都要當成
  待測程式碼看待。

#### 延伸學習
- **Silent failure vs loud failure**：這次三個量測 bug 的共同點是「回傳了合理但錯誤的值」。
  設計量測工具時，「拿不到資料」必須與「資料為零」在型別上可區分（`None` vs `0`），
  今天 `conversation_id` 用 `None` 才讓警告加得上去。
- **Contract consistency over local optimisation**：#85 反轉了一行刻意的註解
  「攔截後不再是結構化輸出」。單看攔截路徑，回純文字很合理；放到 API 契約看，
  「同一支 bot 有時回物件有時回字串」把成本轉嫁給每一個串接方。
  搜尋關鍵字：Postel's law 的反面、schema stability、defensive parsing。
- **分批實驗與對照臂**：跨時間補資料時要同時重跑一個已測過的臂當 control，
  用來偵測平台或供應商模型的漂移。這是實驗設計的常識，但軟體評測常常忘記。
...
## 2026-08-17 — 防護機制做減法：把「語意閘門」併進本來就會跑的那次 LLM

**背景**：競品實測（OPENPOINT / 萬家福官方 LINE）與自家 9 題攻擊後盤點，發現防護是一層層疊上去的：regex 25 條、LLM input guard、LLM output guard、E3 預取（失效）、5 份重複的通路 prompt。使用者的訴求是「不要功能越疊越多」。
**做法**：
1. 減法先行（獨立 commit，行為不變）：拔 E3 預取、拔兩個 LLM guard 執行路徑與 UI、通路規範（格式/長度/角色鎖）抽成 `LINE_CHANNEL_PROMPT_SUFFIX` 由程式注入一次、regex 去角色扮演類。
2. 再加閘門：意圖分類器輸出從兩行擴成三行（類別 / 清洗+改寫查詢 / ATTACK|OK），純攻擊 → 程式回 DB 固定文案、不進檢索與生成；混合型剝掉語氣要求、留下業務問題繼續分流。
**教訓**：
- **加機制前先問「哪一次 LLM 呼叫已經在跑、能不能順便做」** — 分類器每則都跑，把清洗員併進去是零延遲；另開節點就是 +1s。
...
## 2026-09-04 — 異常控管與身分綁定的後台頁：「只送有改的鍵」與「只顯示一次的秘密」

**背景**：Issue #68 P7 前端收尾。設定層是三層覆寫（平台 / 方案 / 租戶），後端 PUT 以「整份覆寫 map」取代；身分綁定的 secret 只能在輪替當下回傳一次。

**做得好**：
1. **表單狀態 = 覆寫 map，而非有效值**：草稿只放使用者真的設過的鍵，繼承值以 placeholder 呈現、每個鍵有「還原」；送出即為完整覆寫 map，與後端「整份取代」語意一致，不會把繼承值誤寫成覆寫。
2. **有效值表帶來源標記**：租戶覆寫 / 方案 / 平台覆寫 / 預設四種來源一眼可辨，管理員排查「為什麼這租戶是 enforce」不必開 DB。
3. **一次性秘密的 UI 契約**：輪替走 AlertDialog 二次確認（舊 secret 立即失效），成功後以獨立 Dialog 顯示 + 複製，關閉即不可再看；狀態查詢永遠只回 has_secret，前端沒有任何路徑能再取回明文。

**隱憂**：
- 「系統預設」分頁的繼承提示用的是 effective_default（已含平台覆寫），API 未暴露程式原始預設；還原後看到的仍是覆寫前的值。→ 補 `program_defaults` 欄位 → 優先級：低。
...
## 2026-09-07 — 雙軌計價與額度用盡策略：把「數什麼」和「用完怎麼辦」拆成兩個開關（Issue #74）

**背景**：原本只有 token 制且用完一律自動加購，商業決策寫死在程式裡。Larry 定案：預設 token、方案可開點數制；用盡策略獨立於計價模式，可切自動展延或用完即擋。

**做得好**：
1. **token 永遠是事實來源**：points 在記帳當下依方案換算寫入 usage 列，歷史不重算，切換方案只影響之後；帳務可回溯，匯率調整不會改寫過去。
2. **兩個正交開關**：billing_mode 決定配額數什麼，exhaustion_policy 決定用完怎麼辦；方案設預設、租戶在方案允許下覆寫，任何切換寫稽核。四種組合都是同一條程式路徑。
3. **預檢單點、三通路 + 背景任務共用**：QuotaPreflightService 放在 abuse gate 旁邊，web / widget / LINE / 文件處理 / 評估跑批同一份判斷；被擋以 QuotaExhaustedError 一種例外對應 402、SSE 事件、LINE 文字、文件狀態四種呈現，通路端只做翻譯。
4. **fail-open 與快取**：Redis 30 秒快取、記帳後失效；查不到一律放行並記 warning，寧可多扣不可斷服務。
5. **自動展延有月上限**：與 P7 異常控管互為表裡，異常流量不會無限加購。

**隱憂**：
- 點數制的 80% / 100% 提醒仍以 token 比例計算；全租戶配額列表沒有點數欄。→ 下一個小 issue → 優先級：中。
- 預估端點只用匯率換算，實際記帳優先模型點數表，兩者在有設模型點數的情況下會有差。→ 預估也讀模型表 → 優先級：低。
- 預檢多一次配額查詢，快取失效後首句會慢幾十毫秒；POC 無 Redis 時每句都查。→ 觀察 → 優先級：低。
...
## 2026-09-09 — 預設值 vs 強制覆蓋：`mode` 這個欄位讓 UI 開始說謊

**Sprint 來源**：Issue #91 收尾後盤點「目前的 prompt 機制有哪些、真正生效的有哪些」時發現。

**本次相關主題**：組態分層、預設值的正確形狀、UI 與行為的一致性

### 發現

`bot.mode`（fast / deep / kb）是**執行期強制覆蓋**：kb 模式在 `_apply_fast_lane` 把
`allow_rerank` 設為 False、`retrieval_modes` 寫死 `["raw"]`、記憶載入與抽取直接 return。
於是後台的 rerank 開關**可以打開、存檔成功、實際完全不生效，而且沒有任何提示**——
模式選擇器只在選 fast 時顯示「rerank / 查詢改寫 / HyDE 會自動關閉」，選 kb 時那行不顯示。

盤點 `mode` 實際強制的七件事後，發現**其中六件早就有各自的獨立欄位**
（`direct_retrieval` / `memory_enabled` / `rerank_enabled` / `query_rewrite_enabled` /
`hyde_enabled` / `enabled_tools`），唯一沒有對應欄位的只有「未命中不升級 ReAct」。
換句話說，`mode` 覆蓋的幾乎都是「已經可以組合表達」的東西，它製造的是重複的真相來源，
不是必要的抽象。

### 正確形狀（專案裡已有先例）

防護階段的分層是對的：**平台底線 / 方案預設 / 租戶 / bot** 四層，
方案（profile）是**預設值**，只有 `required_stages` 是真正不可下修的底線。
`mode` 應該套同一個模式——從「執行期覆蓋」降級為「**存檔時的預設填值**」：

- 建 bot 時選情境預設，一次把各開關填好（解決「八個開關我該關哪些」的探索問題）
- 存檔後開關就是真的，UI 顯示什麼就是什麼
- 使用者要偏離預設隨時可以，不必等平台開新模式
- 只需新增一個欄位（未命中行為：回話術 / 升級推理）補上唯一缺的語意

### 判準

**「這個設定是預設值還是不變式？」**——不變式才可以覆蓋使用者的輸入，而且必須讓使用者
看得出來被覆蓋了（像 `required_stages` 會回報 source=required）。預設值只能在寫入當下
填值，不能在執行期偷偷蓋掉。本次 Issue #91 的 `SECURITY_CLAUSE` 是前者（安全不變式，
連 DB 都不放），`mode` 的六個開關是後者（被錯當成不變式）。

### 潛在隱憂（若要動手）

- 現有 32 隻 bot 全是 kb 模式，migration 必須把各欄位實際填成 kb 對應值，否則改完行為位移。
  這是整件事風險最高的一步，要先在 local-docker 驗過再上 POC → **優先級：高**
- 純組合會讓無效組合變可能（例：不給工具卻要求未命中升級 ReAct）。
  **正解是在 UI 選取當下就不讓它成立**——每個控制項宣告自己的前置條件，前置未滿足時
  disabled 並顯示「要先開什麼」，而不是等按下儲存才回錯誤。使用者從頭到尾建構不出無效狀態，
  理由也出現在做決定的地方。後端仍保留驗證（API 可被直接呼叫），但只需簡單守門，
  不必承擔引導使用者的責任 → **優先級：中**
- 短期成本最低的止血：把「rerank / 改寫 / HyDE 自動關閉」那行提示的顯示條件從
  `mode === "fast"` 放寬為 `fast || kb`，或在 kb 模式直接停用該開關 → **優先級：中**

### 延伸學習

- **Configuration override 的三種語意**：預設值（default）、覆寫（override）、不變式（invariant）。
  三者在 UI 上必須長得不一樣——預設值可改、覆寫要顯示「被誰蓋掉」、不變式要 disabled 並說明原因。
  把三者混成同一個欄位就會出現「開關無效但沒人知道」。可對照 *effective configuration* 的呈現慣例。
- **討論題**：如果 `mode` 降級為預設填值，未來要新增「展覽模式」這種情境，是不是就完全不必動
  管線程式碼、只要多一組預設值？（這正是 channel-parity 規範講的「新通路 = 新轉接器 + 一張能力表，
  管線零修改」的同構問題。）
...
- **`base_prompt` 已於同日移除（後續 commit）**：先前判斷「刪要動 64 檔、會拆掉 prompt 閘門版控／優化器目標／稽核欄位」，實際做下來的成本是後端 15 檔＋測試 30 檔＋前端 8 檔，且稽核能力完全不減（`bot_prompt` / `worker_prompt` / 記憶萃取等長文字欄位全數保留，顯示名稱表對未列欄位會退回原欄位名，歷史紀錄不炸）。**線上零依賴**：POC 上 0 個 bot 有非空值、81 筆稽核 0 筆提及、2 筆版本快照值為空、評測集 0 筆。教訓是「爆炸半徑」要用實際資料量測而不是用引用次數估——`base_prompt` 出現 43 次，但真正有語意的只有 6 個位置，其餘是測試建構參數。
