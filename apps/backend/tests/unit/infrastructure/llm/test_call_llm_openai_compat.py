...
def test_openai_blocks_split_by_role_to_system_and_user_messages():
    """system + user blocks → 兩條 messages（系統先 user 後）。"""
    mock_resp = _mock_httpx_response()
    cm, mock_client = _patch_httpx(mock_resp)
    with cm:
        blocks = [
            PromptBlock(text="You are helpful.", role=BlockRole.SYSTEM, cache=CacheHint.EPHEMERAL),
            PromptBlock(text="Big stable context...", role=BlockRole.USER, cache=CacheHint.EPHEMERAL),
