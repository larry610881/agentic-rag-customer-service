...
class SplitPdfUseCase:
    ...
    async def execute(self, parent_doc_id: str, task_id: str) -> None:
        ...
        # Load PDF from storage；2026-09-08：儲存端讀不到（GCS 權限 / 網路）時，
        # 上傳時同時存進 DB 的原始內容副本可以撐住，只有沒副本才算失敗
        try:
            raw_content = await self._file_storage.load(parent.storage_path)
        except Exception as e:
            if parent.raw_content:
                logger.warning(
                    "split_pdf.load_failed_fallback_db",
                ...
                logger.error(
                    "split_pdf.load_failed", doc_id=parent_doc_id, error=str(e)
                )
        ...
        for png_bytes in iter_pages_as_images(raw_content):
            ...
            # Save PNG to GCS；寫不進去（權限 / 網路）時把頁面圖留在 DB 副本，
            # process_document 對 storage_path 為空的文件會直接讀 raw_content。
            # 圖卡（簽名網址）在此情況下不可用，但 OCR / 分塊 / 向量化照常。
            child_raw = b""
            try:
                ...
                logger.warning(
                    "split_pdf.child_save_failed_fallback_db",
                    doc_id=parent_doc_id,
                    page=page_num,
                    error=str(e)[:200],
                )
