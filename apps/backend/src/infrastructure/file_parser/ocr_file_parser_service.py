...
class OcrFileParserService(FileParserService):
    ...
    def parse(self, raw_bytes: bytes, content_type: str) -> str:
        ...
        return asyncio.run(self._ocr_all_pages(page_images))

    async def _ocr_all_pages(self, page_images: list[bytes]) -> str:
        tasks = [self._ocr.ocr_page(img) for img in page_images]
