...
export function FeedbackStatsSummary({
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {cards.map((card) => (
        <Card key={card.title} className="hover:border-primary/30 transition-colors duration-200">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold font-heading tracking-wide">{card.value}</div>
          </CardContent>
        </Card>
      ))}
    </div>
